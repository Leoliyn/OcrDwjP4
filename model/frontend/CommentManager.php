...
class CommentManager extends Manager {
    ...
    public function getComment($commentId) {
        $db = $this->dbConnect();
        $comment = $db->prepare('SELECT COMM_ID, COMM_PSEUDO, COMM_CONTENU, DATE_FORMAT(COMM_DATE, \'%d/%m/%Y à %Hh%imin%ss\') AS comment_date_fr FROM comments WHERE COMM_ID= ? AND DISABLE= ? ORDER BY comment_date DESC');
